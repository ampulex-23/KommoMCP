...
### 💡 Бизнес-инсайты (kommo_insights)

Аналитика клиентов и возможностей:

| Action | Описание | Ключевые параметры |
|--------|----------|-------------------|
| `top_clients` | Топ клиентов по выручке | limit, by (companies/contacts) |
| `rfm` | RFM-сегментация клиентов | limit, by |
| `workload` | Нагрузка на менеджеров | - |
| `opportunities` | Возможности для допродаж | days_inactive, limit |
| `big_deals` | Крупные сделки в воронке | threshold, limit |

**Примеры:**
- "Топ-10 клиентов" → `action: top_clients, limit: 10`
- "RFM-анализ" → `action: rfm`
- "Нагрузка на менеджеров" → `action: workload`
